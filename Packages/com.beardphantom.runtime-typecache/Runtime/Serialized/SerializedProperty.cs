...
namespace BeardPhantom.RuntimeTypeCache.Serialized
{
    ...
    internal class SerializedProperty : ISerializableMemberInfo<PropertyInfo>
    {
        ...
        {
            Type declaringType = DeclaringType.Deserialize(typeStore);
        ...
        {
            bool canRead = memberInfo.CanRead;
            bool canWrite = memberInfo.CanWrite;
            if (canRead && canWrite)
            {
                return memberInfo.GetMethod.IsPublic || memberInfo.SetMethod.IsPublic;
            }
        ...
        private bool IsPropertyStatic(PropertyInfo memberInfo)
        {
            ...
            if (canRead && canWrite)
            {
                return memberInfo.GetMethod.IsStatic || memberInfo.SetMethod.IsStatic;
            }
